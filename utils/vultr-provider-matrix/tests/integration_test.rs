...
fn test_end_to_end_processing() {
    ...
    // Process the files
    let results = inout::find_table_dump_files(&args.directory).expect("Failed to find test files");
    ...
    if let Some(pass_result) = processed_results.get("vtr_pass_all_checks") {
        assert_eq!(
            pass_result.failed_checks.len(),
            0,
            "pass_all_checks should have no failed checks; has: {:?}",
            pass_result.failed_checks.keys()
        );
        // Should have 5 peers (one for each route)
        assert_eq!(
            pass_result.peers.len(),
            4,
            "pass_all_checks should have 4 peers"
        );
    } else {
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_route_missing_neighbor") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("RouteMissingNeighbor"),
            "fail_route_missing_neighbor should have RouteMissingNeighbor failures"
        );
        assert!(
            fail_result.failed_checks["RouteMissingNeighbor"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_customer_large_comm") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("CustomerWithLargeCommunity"),
            "fail_customer_large_comm should have CustomerWithLargeCommunity failures"
        );
        assert!(
            fail_result.failed_checks["CustomerWithLargeCommunity"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_neighbor_inconsistency") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("NeighborAsnInconsistency"),
            "fail_neighbor_inconsistency should have NeighborAsnInconsistency failures"
        );
        assert!(
            fail_result.failed_checks["NeighborAsnInconsistency"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_route_inconsistent_types") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("RouteInconsistentPeerTypes"),
            "fail_route_inconsistent_types should have RouteInconsistentPeerTypes failures"
        );
        assert!(
            fail_result.failed_checks["RouteInconsistentPeerTypes"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_as_inconsistent_types") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("AsInconsistentPeerTypes"),
            "fail_as_inconsistent_types should have AsInconsistentPeerTypes failures"
        );
        assert!(
            fail_result.failed_checks["AsInconsistentPeerTypes"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_inconsistent_large_comm") {
        assert!(
            fail_result
                .failed_checks
                .contains_key("InconsistentLargeCommPeerAsns"),
            "fail_inconsistent_large_comm should have InconsistentLargeCommPeerAsns failures"
        );
        assert!(
            fail_result.failed_checks["InconsistentLargeCommPeerAsns"] > 0,
    ...
    if let Some(fail_result) = processed_results.get("vtr_fail_json_parse") {
        assert!(
            fail_result.failed_checks.contains_key("JsonParseFailure"),
            "fail_json_parse should have JsonParseFailure failures"
        );
        assert!(
            fail_result.failed_checks["JsonParseFailure"] > 0,
    ...
    inout::write_peers_json(&args, &processed_results).expect("Failed to write output");
